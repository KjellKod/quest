...
## V2 Retrospective & V3 Direction (2026-03-05)
...
### Key Insight: Celebration Data Should Be Collected During the Quest

The fundamental problem is trying to reconstruct rich celebration data from artifacts after the fact. Instead:

**The orchestrator should amass celebration data as the quest progresses:**

- After planning: record plan summary, iteration count, arbiter verdict
- After building: record builder model, files changed, test results
- After review: record reviewer findings, specific issues found/fixed, review verdicts
- After fixing: record what was fixed, how many iterations
- After PR: record PR number, CI status, comments posted, review comments received
- Quest-specific achievements should be **generated by the orchestrator** who has context, not by a generic post-hoc script

This data accumulates in a **celebration data file** (e.g., `.quest/<id>/celebration.json`) throughout the quest. The celebrate script then just renders it — it doesn't need to reconstruct anything.
...
1. **Orchestrator writes celebration.json** at each phase transition in the quest workflow
2. **Celebrate script renders from celebration.json** — no more reconstructing from artifacts
3. **Additive updates** — if you push back on completion, or check PR comments later, the data can be appended to celebration.json and re-rendered
4. **Quest-specific achievements** generated by the orchestrator with full context, not generic pattern matching
5. **Quality label** not just a percentage — use named tiers: Bronze, Silver, Gold, Platinum, Diamond
6. **Victory narrative** — a 3-5 line paragraph summarizing what the quest proved/demonstrated
7. **Domain-specific metrics** — not just file counts, but meaningful stats the orchestrator observed
8. **Big chunky block letters** that actually work — test with real quest names, handle the common cases
9. **Minimal/standard need love too** — minimal should be a good one-liner with key stats, standard should be a satisfying summary box
