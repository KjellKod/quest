...
### gpt
**Purpose:** Delegate tasks to OpenAI Codex through the Quest CLI runner from Claude-led sessions. Provides structured invocation with sensible defaults for sandbox, model, and reasoning effort.
...
**Not for:** Codex-led Quest role dispatch — Codex-led Codex roles use local Codex subagents, and the CLI runner is only for Claude-led dispatch to Codex. See `.skills/gpt/SKILL.md` and the dispatch matrix in `.skills/quest/delegation/workflow.md`.
