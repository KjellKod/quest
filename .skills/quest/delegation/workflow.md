...
After every subagent invocation, the orchestrator MUST verify that the expected artifact file(s) exist on disk:

1. Check that each file listed in the handoff `artifacts` array exists
2. If a file is missing, attempt to extract its content from the subagent response and write it to the expected path
3. If extraction fails, log the missing artifact and surface the error to the user

This is critical for OpenCode where `task` tool subagents may return content in their response without writing to disk.
...
**Format:**
```
<timestamp> | phase=<phase> | agent=<agent_name> | runtime=claude|codex|opencode | iter=<plan_iteration or fix_iteration> | handoff_json=found|missing|unparsable | source=handoff_json|text_fallback
```

Use `plan_iteration` for plan/plan_review phases, `fix_iteration` for code_review/fix phases, and `1` for build (single pass).
Set `runtime` to the runtime actually used for that invocation (`claude`, `codex`, or `opencode`).
...
- Log `runtime=opencode` when invocation used OpenCode `task(agent=...)` tool.
...
2026-02-15T00:15:00Z | phase=plan_review | agent=reviewer_b | runtime=codex | iter=1 | handoff_json=missing | source=text_fallback
...
2026-02-15T00:28:00Z | phase=plan_review | agent=reviewer_b | runtime=codex | iter=2 | handoff_json=found | source=handoff_json
...
4. **Invoke BOTH Plan Reviewers IN PARALLEL** (same message, one Task call + one Codex call):
   ...
   - **Slot A** (Claude): `Task(subagent_type="plan-reviewer")` → `.quest/<id>/phase_01_plan/review_reviewer_a.md`
   - **Slot B** (Codex): `mcp__codex__codex` → `.quest/<id>/phase_01_plan/review_reviewer_b.md`
...
4. **Invoke BOTH Code Reviewers IN PARALLEL** (same message, one Task call + one Codex call):
   ...
   Two different models review independently for model diversity:
   - **Slot A** (Claude): `Task(subagent_type="code-reviewer")` → `.quest/<id>/phase_03_review/review_reviewer_a.md`
   - **Slot B** (Codex): `mcp__codex__codex` → `.quest/<id>/phase_03_review/review_reviewer_b.md`
...
The orchestrator identifies its runtime to select the correct tool invocation pattern:
- **Claude Code**: Has `Task` tool with `subagent_type` parameter
