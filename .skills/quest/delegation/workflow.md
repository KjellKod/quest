...
| Claude-led | Claude | native `Task(...)` | Use the orchestrator's native Claude task path. Effort comes from the subagent's `effort:` frontmatter, which `scripts/quest_sync_model_defaults.py` generates from the allowlist — native tasks cannot read `orchestration.json`, so do not try to pass effort here. |
