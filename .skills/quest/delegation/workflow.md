...
7. **Postflight artifact-path validation (sub-agent roles).** After the orchestrator has read the handoff and processed routing fields, and before the agent response context is discarded for sub-agent roles, the orchestrator invokes `quest_artifact_postflight.py` against the just-accepted handoff. On exit code `0`, continue normally. On non-zero exit, mark the handoff `accepted_with_warnings`, surface the structured mismatch records **from the validator's stdout** to the user, and continue (advisory in this quest; auto-halting is a follow-up). `.quest/<id>/logs/path_compliance.log` is the append-only audit trail across every invocation in the quest; do not read it for current-run attribution because earlier roles / retries / iterations would be misattributed to this handoff. The stdout output is one JSON line per current-run mismatch and is empty on a clean exit. This step applies only to sub-agent roles whose `expected_artifacts_for_role(...)` returns a non-empty path set.
