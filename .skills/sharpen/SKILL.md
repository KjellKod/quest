...
2. **Temperature.** "What's the brand color, if any? Recommended: no brand yet → cool (Tailwind `slate`). Linear, Stripe, Vercel, shadcn default. Override: warm brand → `stone`; utilitarian / explicitly monochrome → `neutral`; tech/scientific → `zinc`; content-forward / paper feel (Google/GitHub) → `gray`."
