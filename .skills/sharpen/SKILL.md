...
2. **Temperature.** "What's the brand color, if any? Recommended: no brand yet → cool (Tailwind `slate`). Linear, Stripe, Vercel, shadcn default. Override: warm brand → `stone`; utilitarian / no brand → `neutral`; tech/scientific → `zinc`; content-forward / paper feel (Google/GitHub) → `gray`."
...
4. **Content-vs-chrome ratio.** "Does the user spend more time looking at the toolbars and panels (chrome-dense — Linear, Figma) or at the content inside them (content-forward — GitHub, Google, a settings form)? Recommended: content-forward for almost everything except dedicated creator tools."
5. **Brand accent.** "Have a brand accent color? Recommended: `#2563eb` (Tailwind `blue-600`) if no other answer — ships with shadcn, reads as 'modern web'. Override: any hex."
