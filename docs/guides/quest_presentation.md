...
**The transport scripts themselves (`quest_claude_bg_run.py` and `quest_claude_bridge.py`) are Quest-agnostic.** They keep transport mechanics separate from Quest phase policy. The Quest-specific behavior (handoff polling, context health logging, text fallback extraction, and parked-session relay) lives in `quest_claude_runner.py` and the workflow docs.
