---
title: Step 2 — Wire the claude --bg transport into Quest (bg default, explicit bridge)
purpose: Executable implementation plan for making the background-agent transport the default for Codex-led Claude roles, with user decision on bg failure, explicit bridge opt-in, transport visibility in the quest summary/celebration, ideas archival, and self-archival on completion.
...
# Step 2 — Wire the `claude --bg` transport into Quest (bg default, explicit bridge)
...
This plan wires the bg transport into Quest as the **default** (`auto`: bg when preflight proves it; if it fails, stop for a user decision instead of silently using bridge), makes the transport **visible** in the quest end summary and celebration (only when Codex called Claude), archives the superseded ideas, and archives itself when done.

Current status: the policy, diagnostics, and Claude Code 2.1.191 prompt delivery fix are wired. The runner sends the initial bg prompt on stdin because positional argv prompt delivery registers a session but can park it at `idle — send a prompt to start`.
...
**Supersedes one spec detail:** `docs/implementation/claude-bg-transport-migration.md` (written pre-Step-1) proposed reimplementing dispatch/confirm/teardown (`dispatch_bg()`, T1–T5) *inside* `quest_runtime/claude_runner.py`. Step 1 already encapsulates all of that in `claude_bg_run.py` with the same file contract and distinct exit codes. **DRY/KISS: invoke `claude_bg_run.py` as a subprocess exactly the way the bridge is invoked** — one selector, two argv builders, same poll/classify/health-log machinery. The spec also predates the 2.1.x management-command churn: the runner uses transcript/pid fallbacks and does not assume `claude logs <id>` or `claude stop <id>` are available without a capability-probed implementation.

## Key design decisions (read before coding)

1. **Quest passes `--handoff-file` to `claude_bg_run.py`.** A `needs_human` handoff makes the bg runner exit promptly and leave the session parked so Quest can resume the same conversation with `--resume <session_id> --answer-file <answer_file>`. Callers without a resume loop can still opt into `--teardown-on-needs-human`.
2. **Transport is resolved by scripts, never by agent prose.** `claude_role_transport` config enum `auto | background-agent | bridge`; `auto` resolves to background-agent after preflight proves it. Failed bg setup stops for a user decision; `bridge` is the explicit API-billing / fully-automated path.
...
| `scripts/quest_runtime/orchestration.py` (`write_orchestration_json`, `:328-351`) | copy the key into `orchestration.json`; add `claude_transport_resolved`; retain `claude_transport_downgraded: false` as a compatibility field for older consumers. Backfill in `migrate_from_snapshot()` like `review-arbiter` is backfilled. |
...
2. Selector: for `--orchestrator codex`, run the configured probe. Under `auto`, run bg only; on failure output `available:false` with remediation and do not probe bridge. Output JSON gains `"transport": "background-agent" | "bridge"` and keeps all existing fields (`available`, `checks`, `cache`, `diagnostic`, `warning`).
...
1. `build_bg_cmd()` next to `build_bridge_cmd()` (`:177-204`), ~25 lines:
   `python3 scripts/claude_bg_run.py --json --no-protocol --prompt-file <p> --name quest-<id>-<agent>-i<iter> --model <concrete-model> --permission-mode <mode> --timeout <t> --handoff-file <handoff> --wait-for <handoff> [--wait-for <artifact>…] [--add-dir <d>…]`. Exact `model=claude` is a sentinel that omits the CLI `--model` flag.
...
3. Exit-code → `classify_result_kind` mapping for the bg child (`claude_bg_run.py` EXIT_* constants):
   ...
   - `5` → `timeout`; `6`, `130`, anything else → fall through to the standard handoff-state classification (typically `handoff_missing`) so the existing missing-handoff retry ladder applies unchanged. `10` is the intentional `needs_human` relay path when `--handoff-file` sees a question.
...
5. `scripts/quest_claude_runner.py`: add `--transport auto|background-agent|bridge` (default `auto`). Resolution for `auto`: background-agent only; startup preflight is responsible for proving bg or stopping for a user decision. Echo the resolved transport in the output JSON envelope (`"transport": "..."`).
...
1. `.skills/quest/delegation/workflow.md` Step 7 context-health reflection (`:1183-1237`): after the compliance block, add:
   ...
   Claude transport (Codex-led roles only):
     background-agent: <N>    bridge: <N>    [legacy downgrade flag: yes/no — from orchestration.json claude_transport_downgraded]
   ```
...
2. `.ai/quest.md` (`:77-82`): rewrite the Codex-led note: bg default via `auto`, explicit bridge/API path, `quest_claude_runner.py` unchanged as entrypoint. (Also fix the existing typo `claude_cli_bridge.py` → `quest_claude_bridge.py`.)
3. `scripts/quest_claude_bridge.py`: docstring note only — "explicit API transport; default for CI, `ANTHROPIC_API_KEY`, and daemonless contexts". No code change.
...
1. `tests/unit/test_quest_runtime.py` (extend): transport resolution matrix `{auto, background-agent, bridge}` → `{bg, bg, bridge}`; `build_bg_cmd` argv pinned (incl. `--no-protocol`, `--handoff-file`, name scheme, model sentinel omission); bg exit-code/status → result_kind mapping; `append_context_health_log` emits `transport=` only when given.
...
4. `tests/test-quest-preflight.sh` (extend): shim scenarios — bg probe ok → `transport=background-agent`; `agents` prints non-JSON (old CLI) under `auto` → `available=false` with bridge opt-in guidance; explicit bridge → `transport=bridge`.
...
Code/config:
- [ ] `claude_role_transport` flows allowlist → schema → orchestration.json, with `claude_transport_resolved` recorded at preflight and `claude_transport_downgraded` retained as false compatibility metadata.
- [ ] `quest_claude_runner.py --transport` works for all three values; `auto` resolves to background-agent only; failed bg preflight **blocks loudly** for user decision (never silently bridges); explicit `bridge` untouched behavior.
...
5. **Bg failure drill**: temporarily break bg (e.g. `CLAUDE_CONFIG_DIR=/tmp/empty scripts/quest_preflight.sh --orchestrator codex`, or run where the daemon can't start) with `auto` → preflight reports `transport: background-agent`, `available:false`, and warning text that asks the user to fix bg or explicitly choose bridge. *Fail signal:* automatic bridge use.

## Out of scope (YAGNI — value over good-to-have)

- Historical note: the first Step 2 wiring deferred the `needs_human` resume relay and shipped only measurement. The current bg transport hardening work wires the relay through `quest_claude_runner.py --resume <session_id> --answer-file <answer_file>` for background-agent sessions; remaining out-of-scope work is richer parked-session UX beyond the file/state contract.
