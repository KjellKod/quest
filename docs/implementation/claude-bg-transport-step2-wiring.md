...
1. `tests/unit/test_quest_runtime.py` (extend): transport resolution matrix `{auto, background-agent, bridge}` → `{bg, bg, bridge}`; `build_bg_cmd` argv pinned (incl. `--no-protocol`, no `--handoff-file`, name scheme); bg exit-code → result_kind mapping (2/3/4/5/6/10/130); `append_context_health_log` emits `transport=` only when given.
