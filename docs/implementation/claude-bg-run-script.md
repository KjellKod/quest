...
|---|---|---|
| F1 | `claude logs <id>` is not treated as a stable scripting dependency. Earlier 2.1.x builds lacked a scriptable logs command and current behavior should be capability-probed before adoption. The runner treats the transcript at `~/.claude/projects/<project>/<sessionId>.jsonl` as the portable log source. | Results MUST travel via files. Diagnostics (`logs_tail`) come from the transcript JSONL (last assistant-text lines, distilled), located by globbing `<transcripts-root>/*/<sessionId>.jsonl`. Direct `claude logs <id>` adoption requires a future capability-probed change. |
...
| F9 | `claude stop <id>` / `claude rm <id>` are not treated as stable scripting dependencies. Earlier 2.1.x builds parsed unknown management verbs as prompts and silently did nothing; current behavior should be capability-probed before adoption. The runner uses the portable fallback: the `agents --json` row carries the session's `pid`, so teardown signals that pid. The daemon may **respawn a parked session once** from its spare pool (same row id, fresh pid), and a killed row may linger pid-less ("settled") in the listing. Background sessions auto-isolate edits into `.claude/worktrees/` unless disabled. | Teardown = signal the row's *current* pid (SIGTERM, escalate to SIGKILL) **repeatedly until the row drops its pid or disappears**, only **after** results are collected. Pass `--settings '{"worktree":{"bgIsolation":"none"}}'` so writes land in the real workspace. Direct management-subcommand adoption requires a future capability-probed change. |
