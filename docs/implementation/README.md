...
| [claude-bg-transport-migration.md](claude-bg-transport-migration.md) | active — spec | Step 2 rationale + transport contract: migrate Codex-led Claude role execution from the `claude --print` bridge (API-metered after June 15, 2026) to the background-agent transport behind a config switch; bridge demoted to fallback/CI path, not deleted. |
| [claude-bg-transport-step2-wiring.md](claude-bg-transport-step2-wiring.md) | active — in progress | Step 2 execution plan: `claude_role_transport` config (`auto` default), preflight bg probe + loud bridge fallback, `build_bg_cmd` wiring under `quest_claude_runner.py`, transport callout in summary/celebration, ideas archival, human runbook, self-archival AC. |
