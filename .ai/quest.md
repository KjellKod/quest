...
- `effort` — reasoning effort per role, applied on whichever runtime the role's model selects. Run `scripts/quest_sync_model_defaults.py` after editing `models` or `effort`; the generator is what keeps the four dispatch surfaces in step.
